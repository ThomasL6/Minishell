...
void ft_double_redir(char **av, t_base *base, int j)
{
	...
	while (base->tableau[j][++i] && ft_strncmp(base->tableau[j][i], ">>", ft_strlen(base->tableau[j][i])) != 0)
	{
	...
	base->fd_out = open(base->tableau[j][++i], O_WRONLY | O_CREAT | O_APPEND, 0644);
	base->flag_redir = 0;
}
...
void ft_basic_redir(char **av, t_base *base, int j)
{
	...
	i = 0;
	tmp = malloc(sizeof(char **) * (ft_tablen(base->tableau[j])));
	while (ft_strncmp(base->tableau[j][i], ">", ft_strlen(base->tableau[j][i])) != 0 && base->tableau[j][i])
	{
	...
	i++;
	base->fd_out = open(base->tableau[j][i], O_WRONLY | O_CREAT | O_TRUNC, 0644);
	base->flag_redir = 0;
}
